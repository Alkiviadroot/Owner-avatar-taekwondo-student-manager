...
export const load = async ({ locals, params }: any) => {
    ...
    const mathitesProgramatos = serializeNonPOJOs(await locals.pb.collection('programa').getFullList({
        filter: 'mera = "' + params.meraId + '"',
    }));

    let parousies = serializeNonPOJOs(await locals.pb.collection('parousies').getFullList({
        filter: 'apousiologioId = "' + params.apousiologioId + '"',
    }));

    const mera = serializeNonPOJOs(await locals.pb.collection('meres').getFirstListItem(
        'id = "' + params.meraId + '"'
    ));
    ...
    for (const mathitis of mathitesProgramatos) {
        ...
        mathitisObj.fotografiaView = locals.pb.files.getUrl(mathitisObj, mathitisObj.fotografia, { 'token': fileToken });
