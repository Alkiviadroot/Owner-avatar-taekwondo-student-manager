...
export const mathitis = z
	.object({
		...
		tk: z
			.number()
			.min(1000, 'Απαιτεί τουλάχιστον 4 χαρακτήρες')
			.nonnegative('Μη έγκυρος χαρακτήρας ( - )')
			.int('Μη έγκυρος χαρακτήρας ( . ) ')
			.finite()
			.safe(),
		perioxi: z.string().max(64, 'Πάρα πολλοί χαρακτήρες').trim(),
		kinito: z
			.number()
			.min(10000000, 'Απαιτεί τουλάχιστον 8 χαρακτήρες')
			.nonnegative('Μη έγκυρος χαρακτήρας ( - )')
			.int('Μη έγκυρος χαρακτήρας ( . ) ')
			.finite()
			.safe(),
		...
		fotografia: z
			.string()
			.regex(new RegExp(ACCEPTED_IMAGE_TYPES), 'Ο τύπος του αρχείου δεν είναι αποδεκτός')
	})
	.partial({
		diefthinsi: true,
		tk: true,
...
export const deltia = z
	.object({
		mathitis: z.string().trim(),

		forma_GDPR: z
			.string()
			.regex(new RegExp(ACCEPTED_FILE_TYPES), 'Ο τύπος του αρχείου δεν είναι αποδεκτός'),
